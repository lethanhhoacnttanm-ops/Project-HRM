...
const JobCardList = ({ dataJobs, candidateCounts = {}, pagination, pageNumber, setPageNumber, onViewDetail, onNavigateApproval, propState }) => {
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {dataJobs.map((job) => {
          ...
          const currentCandidates = candidateCounts[jobId] || [];
          return (
            <div
              key={job._id || job.id}
            ...
            >
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                <Button
                  onClick={() => onNavigateApproval ? onNavigateApproval(jobId) : null}
                  className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold h-9 flex-1 flex items-center justify-center gap-1.5 border-none"
                >
                  <span>Duyệt CV ({count})</span>
