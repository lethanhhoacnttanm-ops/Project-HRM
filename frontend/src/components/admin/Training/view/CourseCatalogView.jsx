    ...
    <div className="space-y-6 my-4">
      ...
      <div className="relative px-2 sm:px-10">
        ...
        {dataCourse && dataCourse.length > 0 ? (
                ...
                <div>
                  <div className="relative h-36 w-full bg-slate-100 dark:bg-gray-800 overflow-hidden">
                    ...
                    <div className="absolute top-2 left-2">
                      <span className="text-[10px] font-extrabold uppercase bg-slate-900/80 dark:bg-gray-900/90 backdrop-blur-xs text-white px-2.5 py-1 rounded-lg">
                        {course.targetLevel}
                      </span>
                    </div>
                  </div>
                  ...
                  <CardHeader className="p-4 pb-2 space-y-1.5">
                    ...
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white line-clamp-2 leading-snug" title={course.title}>
                      {course.title}
                    </h4>
                  </CardHeader>
                  ...
                  <CardContent className="p-4 pt-0">
                    ...
                    <div className="mt-3 inline-block bg-slate-50 dark:bg-gray-800 border border-slate-100 dark:border-gray-700 px-2 py-1 rounded-md text-[10px] font-semibold text-slate-600 dark:text-gray-300">
                      Vị trí: <span className="text-slate-900 dark:text-white">{course.position}</span>
                    </div>
                  </CardContent>
                </div>

                <CardFooter className="p-4 pt-2 border-t border-slate-100 dark:border-gray-800 dark:text-gray-800">
                  <a
                    href={course.courseUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full flex items-center justify-center gap-1.5 bg-slate-900 hover:bg-indigo-600 dark:bg-indigo-600 dark:hover:bg-indigo-500 text-white text-xs font-semibold py-2 rounded-xl transition-all shadow-2xs"
                  >
          ...
          <div className="text-center py-16 bg-white dark:bg-gray-900 border border-slate-100 dark:border-gray-800 rounded-2xl">
            <BookOpen className="w-8 h-8 text-slate-300 dark:text-gray-600 mx-auto mb-2" />
            <p className="text-xs text-slate-500 dark:text-gray-400 font-medium">Không tìm thấy khóa học nào trong trang này.</p>
          </div>
        )}
      </div>
    </div>
