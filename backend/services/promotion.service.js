...
class PromotionService {
  async getAllPromotion({ page, limit, status }) {
    ...
    const { totalPromotion, dataPromotion } = await promotionRepository.FindWithPagination({
      skip,
      limit: pageSize,
      status
    });
  ...
  async createPromotion(data, adminId) {
    ...
    const savedPromotion = await promotionRepository.createPromotion(promotionPayload);
