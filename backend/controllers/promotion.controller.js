...
class PromotionsController {
  async createPromotion(req, res) {
    try {
      ...
      return res.status(201).json({
        success: true,
        message:
          "Tạo đề xuất thăng tiến thành công và đang chờ đến ngày hiệu lực!",
        data: newPromotion,
      });
    } catch (error) {
  ...
  async updateStatus(req, res) {
    try {
      ...
      return res.status(500).json({
        success: false,
        message:
          error.message || "Lỗi máy chủ khi cập nhật trạng thái đề xuất!",
      });
    }
  }
  ...
  async getMyPromotions(req, res) {
    try {
      const data = await promotionService.getMyPromotions(req.user.id);
      return res.status(200).json({
        success: true,
        message: "Lấy thông tin thăng tiến thành công!",
        data,
      });
    } catch (error) {
      return res.status(500).json({
        success: false,
        message: error.message || "Lỗi khi lấy thông tin thăng tiến!",
      });
    }
  }
}
